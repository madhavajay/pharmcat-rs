...
- Authoritative gate after the 2026-06-04 PipelineTest porting batch (~37 new tests + 2 parity fixes):
  - `cargo test -p pharmcat pipeline::tests` passed: 91 passed, 0 failed, 226 filtered out, finished in 2109s. Every new PipelineTest fixture passes together with no interactions.
  - `cargo test -p pharmcat phenotype::` passed: 30 passed (regression check for the `split_outside_diplotype` sort).
  - `cargo test -p pharmcat report::tests`: 84 passed, 2 failed — the failures are the pre-existing DPYD/RYR1 `match_score` `84`-vs-`166` issue documented under "KNOWN ISSUE" below (confirmed pre-existing by revert-bisect; this session's two production changes are not the cause).
  - `cargo test -p pharmcat cyp2c19` passed: 15 passed (outside-call regression check, full set).
...
- JAVA REFERENCE BASELINE ESTABLISHED (2026-06-04) — first clean full Java run, the named baseline:
  - Command: `GRADLE_USER_HOME="$PWD/.gradle-home" ./repos/PharmCAT/gradlew -p repos/PharmCAT --no-daemon test` (needs a network-enabled / sandbox-disabled shell + writable `GRADLE_USER_HOME`; Gradle 9.0.0, JVM 17.0.18).
  - Result: BUILD SUCCESSFUL in 10m23s. **62 test classes, 426 tests, 0 failures, 0 errors, 0 skipped** (test exec 556s). Reports at `repos/PharmCAT/build/reports/tests/test/` and `build/test-results/test/`.
  - This supersedes the earlier "Java baseline blocked" status throughout this file. Re-run the same command in a clean workspace to update.
- Implementation note from this batch:
  ...
  - Java `PipelineTest.testUgt1a1s80s28missing` is now covered by a Rust UGT1A1 fixture (`rs3064744` missing, `rs887829` `C/T`) asserting the `[*1/*80, *1/*80+*28, *1/*80+*37]` partial match, all three recommendation diplotypes, and the grouped atazanavir CPIC HTML rows. A new `atazanavir_ugt1a1_rx_dip_rows` test helper mirrors Java's `select(".cpic-guideline-atazanavir").select(".rx-dip")` DOM walk and confirms two rows: `[*1/*80]` then `[*1/*80+*28, *1/*80+*37]`. Rust HTML already grouped the recommendations exactly like Java; no new behavior.
  ...
  - Java `PipelineTest.testUgt1a1HG00436` is now covered by a Rust UGT1A1 phased fixture (`rs887829` `T|C`, `rs3064744` `CATAT|CAT`, `rs35350960` `A|C`, all `1|0`) where hap1 carries `*80`+`*28`+`*27` together — no single named allele describes it with combinations disabled — so the matcher returns `NoCall` and the report shows `Unknown/Unknown`. Rust already returned NoCall; no new behavior.
  - Java `PipelineTest.testUgt1a1s1s80s27s60s28MissingPhased` is now covered by a Rust UGT1A1 phased fixture (`rs3064744` missing, `rs887829` `T|C`, `rs35350960` `A|C`) — hap1 = `*80`+`*27` with the `*28` repeat missing — asserting `NoCall`, `Unknown/Unknown`, and that both matcher `match_data.phased` and the report stay phased.
  - Java `PipelineTest.testUgt1a1s1s60s80s6phased`, `testUgt1a1s1s60s80s28s6phased`, and `testUgt1a1s1s37s80s60phased` are now covered by Rust phased fixtures sharing a new `run_ugt1a1_phased_pipeline` helper. The first two are `NoCall` phased cases (`*80`+`*27`, and `*80`+`*28`+`*27`) asserting `Unknown/Unknown` + phased; the third asserts the called `*1/*80+*37` (hap1 = `*80`+`*37` via `rs3064744` `TA(9)` = `CATATAT`) with recommendation alleles `{*1, *80+*37}`. This completes the UGT1A1 `PipelineTest` group (24 methods).
  - Java `PipelineTest` CYP3A5 group (`testCyp3a5v1`/`v2`/`v3`/`v4`/`v5` and `testCyp3a5Missing3Message`) is now covered by Rust fixtures over the main CYP3A5 definition (new `CYP3A5_MAIN_DEFINITION_PATH`) sharing a `run_cyp3a5_unphased_pipeline` helper and an `assert_cyp3a5_call` helper. v1/v4 assert `*1/*3` (`rs776746` `T/C`), v2/v5 assert `*3/*9` (`rs28383479` `C/T` + `rs776746` `C/T`), v3 asserts `*3/*3` (`rs776746` `C/C`). Missing3 asserts `*1/*1` with `rs776746` missing, an unphased report, the missing `rs776746` variant report, and a non-empty tacrolimus CPIC message list. No new behavior required.
  - Java `PipelineTest` HLA-B group (`testHlab`, `testSingleHlabAllele`, `testHlabPhenotype`) is now covered by Rust fixtures over the CYP2C9 definition plus an HLA-B outside call (new `CYP2C9_DEFINITION_PATH`, `run_hlab_outside_pipeline`, `gene_source_phenotypes`, and `assert_matched_from_source` helpers). They assert HLA-B is never matcher-called but is an outside-call report, the derived `*15:02`/`*57:01`/`*58:01` phenotypes (full diplotype, single allele, and phenotype-only outside-call forms), and abacavir/allopurinol/phenytoin matched-annotation counts including the two-gene CYP2C9+HLA-B phenytoin lookup (6 total). `testHlabPhenotype` also covers the phenotype-only outside-call format `HLA-B\t\t*57:01 positive`.
  - PARITY GAP found while porting `testSingleHlabAllele`: Java asserts carbamazepine CPIC == 3, a two-gene (HLA-A + HLA-B) lookup whose recommendations key HLA-A as `No Result` when HLA-A has no data. Java's `Phenotyper` seeds an unspecified (`No Result`) `GeneReport` for every guideline-referenced gene with no input (`Phenotyper.listUnspecifiedGenes` → `GeneReport.unspecifiedGeneReport`), so `Genotype.makeGenotypes` includes `HLA-A=No Result`. The Rust `ReportContext` does not yet seed those unspecified gene reports, so a two-gene lookup with a fully-absent gene returns 0. The carbamazepine CPIC assertion is omitted from the Rust test (the DPWG single-gene assertion is kept) until this is fixed.
  - Java `PipelineTest.testCyp3a4` and `testIfnl3` are now covered by Rust single-gene fixtures sharing a new generic `run_single_gene_unphased_pipeline(definition_path, ...)` helper (with new `CYP3A4_DEFINITION_PATH` / `IFNL3_DEFINITION_PATH`). CYP3A4 asserts `*8/*8` (`rs72552799` `T/T`) and quetiapine == 1 (CYP3A4-only DPWG lookup); IFNL3 asserts the reference call `rs12979860 reference (C)/rs12979860 reference (C)` and zero peginterferon alfa-2a/2b annotations.
  - Java `PipelineTest.testMtrnr1` is now covered by a Rust fixture (CYP2C19 + CYP2C9 reference VCF + `MT-RNR1\tm.1555A>G` outside call) asserting CYP2C19/CYP2C9 are matcher-called, MT-RNR1 is an outside-call report, and amikacin CPIC == 1 / FDA_LABEL == 1 (MT-RNR1-only lookups).
  - Java `PipelineTest.testOutsideCallDiplotypeNormalization` is now covered, AND it surfaced + fixed a real parity gap: outside-call diplotypes were stored in raw order, so `CYP2C19 *2/*1` printed as `*2/*1`. Java's `OutsideCall` stores haplotypes in a `TreeSet` ordered by `HaplotypeNameComparator`. Fix: `split_outside_diplotype` now sorts the two alleles via `compare_haplotype_names` (the same comparator used for matcher report labels), so `*2/*1` normalizes to `*1/*2`. All `phenotype::` unit tests (30) still pass; existing outside-call pipeline tests (15 CYP2C19, full run) are unaffected (they use already-ordered diplotypes).
  - Java `PipelineTest.testOutsideCallActivityScoreAndPhenotype` and `testOutsideCallActivityScore` are now covered by Rust fixtures (CYP2C19 reference VCF via a new `run_cyp2c19_reference_with_outside_pipeline` helper + a CYP2D6 outside call). The phenotype-override case asserts CYP2C19 `*38/*38`, CYP2D6 outside `*2/*10`, and that all CYP2D6 source diplotypes carry the overridden `Intermediate Metabolizer` phenotype. The activity-score-only case asserts CYP2D6 prints `Normal Metabolizer (1.25)`, AND fixed a real parity gap: `source_diplotype_label`'s `(None, None)` branch returned only the phenotype, dropping the score; it now formats `phenotype (score)` like the canonical `report_diplotype_label` (no-score path unchanged, so HLA phenotype-only labels are unaffected).

  - Java `PipelineTest.testWarfarinDpwg` is now covered by a Rust fixture (CYP2C9 `rs1057910` `C/C` → `*3/*3` + VKORC1 `rs9923231` `T/T`, new `VKORC1_DEFINITION_PATH`) asserting both genes are matcher-called, CYP2C9 `*3/*3`, and warfarin DPWG matched annotations == 2 (two separate single-gene DPWG annotations, both genes present so no No Result gap).
  - Java `PipelineTest.testOutsideSinglePositionCalls` is now covered by a Rust fixture (CYP2C9 reference VCF + a two-line outside file: IFNL3 single-position `rs12979860 reference (C)/...` and CYP4F2 `*1/*3`), asserting CYP2C9 is matcher-called, IFNL3/CYP4F2 are outside-call reports (not matcher-called), and their exact source-diplotype labels.
  - Java `PipelineTest.testDiplotypeOverrideRecommendation` is now covered by a Rust CYP2C9 `*2/*2` fixture (`rs1799853` `T/T`) asserting the matcher/source `*2/*2` and that phenytoin DPWG has exactly one matched annotation — the `*2/*2`-specific recommendation overrides the generic poor-metabolizer one. (Java additionally checks the annotation `localId` `DPWG-PA166299254`; the Rust report does not yet expose annotation local IDs, so that assertion is deferred.)

## RESOLVED — pre-existing report-test failures (fixed 2026-06-04 using the Java oracle)

- [x] `report::tests::report_gene_from_dpyd_..._like_java` and `..._ryr1_..._like_java` were failing on the inherited HEAD (`cb05d02`), independent of this session (confirmed by revert-bisect + git log showing all recent commits are test-only). Root cause: the tests hard-coded a **wrong** expected `match_score` computed as `2 x Reference score` (DPYD `166`, RYR1 `626`). The Rust matcher is correct — `DiplotypeMatch.score` sums the two `NamedAllele` `score` fields straight from the definition JSON, exactly like Java: DPYD Reference(83)+c.2846A>T(1)=`84`, RYR1 Reference(313)+c.97A>G(1)=`314`. Verified the matcher emits `84`/`314` and the JSON `score` fields are `83/1` and `313/1`. Fix: corrected both test expectations to `84`/`314` with explanatory comments. Both now pass.
- Next unblocked slice:
  ...
  - Implement unspecified (`No Result`) gene-report seeding in the Rust reporter (scoped 2026-06-04, ready to implement; deferred because it needs the network-blocked Java oracle to arbitrate shifted expected values):
    - Java `Phenotyper.listUnspecifiedGenes(env, source)` = `env.getDrugs().getGenesUsedInSource(source)` minus genes that already have a report (empty for `DataSource.UNKNOWN`). `GeneReport.unspecifiedGeneReport(gene, env, source)` is just `new GeneReport(gene, env)` (a `No Result` report; `CallSource.NONE`).
    - Rust plan: derive the per-source used-gene set from `PgkbGuidelineCollection` (each guideline's `related_genes` grouped by source), subtract genes already in the `ReportContext`, and add a `No Result` report (reuse/extend `ReportGene::unknown`, whose lookup/phenotype is already `No Result`) for the remainder during `ReportContext` construction in `run_reporter_from_vcf`.
    - WARNING / why deferred: this adds many `No Result` gene reports to every report, which can shift HTML/JSON/reportability/gene-count assertions across existing pipeline tests written WITHOUT seeding. Arbitrate every newly-failing count against the Java reference (Gradle, currently network-blocked) before changing expectations. Then restore the carbamazepine CPIC == 3 assertion in `run_reporter_from_vcf_single_hlab_allele_like_java_pipeline_test`.
  - More single-gene `PipelineTest` ports that avoid the No Result gap: `testMtrnr1` (MT-RNR1 outside call + CYP2C19/CYP2C9 reference, amikacin CPIC/FDA_LABEL), and the `testOutsideCall*` group. `testCyp2b6star1star34AllMatch` needs an all-matches (non-top-candidate) matcher mode — confirm `ReporterPipelineOptions` exposes it before porting.
