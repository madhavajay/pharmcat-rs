...
- Implementation note from this batch:
  ...
  - PARITY GAP found while porting `testSingleHlabAllele` — NOW RESOLVED (2026-06-04): Java asserts carbamazepine CPIC == 3, a two-gene (HLA-A + HLA-B) lookup whose recommendations key HLA-A as `No Result` when HLA-A has no data. Fixed by porting `Phenotyper.listUnspecifiedGenes` / `GeneReport.unspecifiedGeneReport` — see the "No Result gene-report seeding DONE" note below. The carbamazepine CPIC == 3 assertion is restored in the Rust test.
...
- No Result gene-report seeding DONE (2026-06-04): `run_reporter_from_vcf` now seeds an unspecified (`No Result`) `ReportGene::unknown` for every gene used by a CPIC or DPWG drug (`PgkbGuidelineCollection::genes_used_in_source(DataSource::Cpic)` ∪ `Dpwg`) that has no matcher or outside data — mirroring Java `Phenotyper.listUnspecifiedGenes` + `GeneReport.unspecifiedGeneReport` (which seed for CPIC then DPWG). A multi-gene recommendation lookup can now key an absent gene as `No Result` (carbamazepine CPIC == 3 on HLA-A+HLA-B with only HLA-B present). Full `pipeline::tests` = 92 passed, 0 failed: zero blast radius, the seeding only added the carbamazepine match. `report::tests` are unaffected (they build `ReportContext` directly).
- Test-suite speedup DONE (2026-06-04): pipeline tests re-parsed the phenotype dir + `prescribing_guidance.json` + message catalog from disk on every test. Added `shared_phenotypes`/`shared_guidance`/`shared_messages` `OnceLock` caches (load once, hand out cheap clones). `cargo test -p pharmcat pipeline::tests` went from ~2109s to ~46s (a 5-test batch from 137s to 17s).
- Next unblocked slice:
  - Build the Java-vs-Rust parity harness now that the Gradle oracle is reachable (`GRADLE_USER_HOME` + network-enabled shell): build/locate the Java reference, run both on identical fixtures, diff outputs with documented normalization. Highest structural value.
  - Remaining `PipelineTest` methods: `testOutsideCallCollision` / `outsideCallCollision2Files` (prefer-sample-data + outside-call messages), `testOutsideCallPhenotypeOverridesDiplotype`, `testNoCallOutsideCall`, `testCyp2b6star1star34AllMatch` (needs an all-matches/non-top-candidate matcher mode), `phaseSetCyp2C9` (phase-set HTML DOM), `testDuplicateEntry*` (HTML warning DOM), `testRecommendationExamples`.
